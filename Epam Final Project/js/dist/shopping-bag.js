...
var area = document.getElementsByClassName("wrapper-purchases")[0];
var spanTotalPrice = document.getElementsByClassName("total-price");
var spanAppliedDiscount = document.getElementsByClassName("applied-discount");
var totalPrice = 0;
var appliedDiscount = 0;
var numItems = 0;
var arrPrices = [];
var discount = 0;
var arrDiscountPrice = [];
...
var arrQuantities = document.getElementsByClassName("quantity");
...
area.addEventListener("click", function (e) {
    if (e.target && e.target.className == "quantity-plus") {
        var idClickedPlus = e.target.id.replace("plus-", "");
        arrQuantities[idClickedPlus - 1].innerHTML = "";
        var dataCount = arrQuantities[idClickedPlus - 1].getAttribute("data-count");
        dataCount++;
...
area.addEventListener("click", function (e) {
    if (e.target && e.target.className == "quantity-minus") {
        var idClickedMinus = e.target.id.replace("minus-", "");
        var dataCount = arrQuantities[idClickedMinus - 1].getAttribute("data-count");
        if (dataCount > 1) {
            dataCount--;
...
function remove(n) {
    ...
    if (signsAfterComma(appliedDiscount) == 1) sumBag.append(sign + appliedDiscount + 0);else sumBag.append(sign + appliedDiscount);
    ...
    if (signsAfterComma(totalPrice) == 1) spanTotalPrice[0].append(sign + totalPrice + 0);else spanTotalPrice[0].append(sign + totalPrice);
    if (numItems == 0) document.getElementsByClassName("text-after-del-purchases")[0].style.display = "block";
}
...
function drawBag() {
    catalog.forEach(function (value) {
        switch (value.id) {
            case "8c061815-6a7d-4465-bb78-1bdc6c5adebf":
                ...
                if (signsAfterComma(value.price) == 1) prices[0].append(sign + value.price + 0);else prices[0].append(sign + value.price);
            ...
            case "ff665110-5e7f-435d-b1b4-651c3d5050ca":
                ...
                if (signsAfterComma(value.price) == 1) prices[1].append(sign + value.price + 0);else prices[1].append(sign + value.price);
            ...
            case "07cf6ce2-6eee-4e78-a441-f257fdea7ed6":
                ...
                if (signsAfterComma(value.price) == 1) prices[2].append(sign + value.price + 0);else prices[2].append(sign + value.price);
            ...
            case "8b300772-eee3-4ff1-b091-e89f17e0e469":
                ...
                if (signsAfterComma(value.price) == 1) prices[3].append(sign + value.price + 0);else prices[3].append(sign + value.price);
    ...
    for (var _i = 0; _i < prices.length; _i++) {
        arrPrices.push(prices[_i].textContent.replace("£", ""));
        arrDiscountPrice.push(itemDiscPrices[_i].textContent);
    }
}